...
class ZappaLoader(object):
    ...
    def __init__(self, settings, api_stage=None, app_function=None, session=None):
        ...
        if not self.settings.get('project_name'):
            self.settings.project_name = slugify.slugify(self.project_dir().split(os.sep)[-1])[:15]

        if len(self.settings.project_name) > 15:  # pragma: no cover
            click.echo(click.style("Warning", fg="red", bold=True) +
                       "! Your " + click.style("project_name", bold=True) +
        ...
        if self.settings.app_function or self.settings.django_settings:
            ...
            with zipfile.ZipFile(self.settings.zip_path, 'a') as lambda_zip:
                ...
                if self.settings.debug:
                    settings_s += "DEBUG=True\n"
                else:
                    settings_s += "DEBUG=False\n"
    ...
    def create_and_configure_apigateway(self):
        ...
        # Create/link API key
        if self.settings.api_key_required:
            if self.settings.api_key is None:
                self.zappa.create_api_key(api_id=api_id, stage_name=self.settings.api_stage)
            else:
                self.zappa.add_api_stage_to_api_key(
                    api_key=self.settings.api_key, api_id=api_id, stage_name=self.settings.api_stage)
